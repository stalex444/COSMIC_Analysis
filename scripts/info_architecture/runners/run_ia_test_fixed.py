...
"""
Fixed Information Architecture Test Runner
This script runs the Information Architecture test with 10,000 simulations
with reliable progress reporting and optimized performance.
"""
...
def update_progress_file(progress_file, sim_index, score, p_value, flush=True):
    """Update the progress file with a new simulation result."""
    try:
        ...
        print("Error updating progress file: %s" % str(e))

def update_status_file(status_file, message, flush=True):
    """Update the status file with a new message."""
    try:
        ...
        print("Error updating status file: %s" % str(e))
...
def run_monte_carlo_simulation(data, constant, num_simulations, output_dir, config):
    """
    Run Monte Carlo simulation efficiently with reliable progress reporting.
    ...
    """
    print("\nStarting Monte Carlo simulation for constant %s" % constant)
    ...
    print("Actual score: %s (calculated in %.2f seconds)" % (actual_score, elapsed))
    ...
    update_status_file(status_file, "Initialized %d worker processes" % num_workers)
    update_status_file(status_file, "Starting time: %s" % datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    
    # Process simulations in batches for better progress reporting
    ...
    try:
        while completed < num_simulations:
            ...
            update_status_file(
                status_file, 
                "Processing batch of %d simulations (total completed: %d/%d)" % 
            ...
            for i in range(current_batch_size):
                ...
                args.append((data, constant, seed, config))
            ...
            for i, score in enumerate(batch_scores):
                ...
                # Update progress file (only flush every 10 simulations to reduce I/O)
            ...
            update_status_file(status_file, progress_str.strip())
    
    except Exception as e:
        # Log any errors
        update_status_file(status_file, "ERROR: %s" % str(e))
        raise
    ...
    update_status_file(status_file, "Completed %d/%d simulations in %.1f seconds" % (completed, num_simulations, elapsed_time))
    update_status_file(status_file, "End time: %s" % datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'output_dir': "../results/information_architecture_10k_fixed"
    }
